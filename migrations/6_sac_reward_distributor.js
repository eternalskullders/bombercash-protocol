const { sacPools, INITIAL_SAC_FOR_POOLS } = require('./pools');
...
module.exports = async (deployer, network, accounts) => {
  ...
  const initialCashAmount = unit.muln(INITIAL_SAC_FOR_POOLS).toString();

  const cash = await Cash.deployed();
  const pools = sacPools.map(({contractName}) => artifacts.require(contractName));
  ...
  console.log(`Deposited ${INITIAL_SAC_FOR_POOLS} SAC to InitialCashDistributor.`);
